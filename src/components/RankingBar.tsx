...
const RankingBar: React.FC = () => {
  ...
  return (
    ...
    >
      <div className={`
        relative backdrop-blur-sm bg-gradient-to-r from-slate-800/60 via-slate-700/60 to-slate-800/60 
        border border-slate-600/30 rounded-xl shadow-lg transition-all duration-300 ease-out
        ${isHovered ? 'shadow-blue-500/10 border-blue-500/20 scale-[1.02]' : ''}
        ${isExpanded ? 'pb-3' : 'pb-1'}
      `}>
        {/* Subtle glasmorphism overlay - more transparent */}
        <div className="absolute inset-0 bg-gradient-to-r from-blue-600/5 via-purple-600/5 to-blue-600/5 rounded-xl" />
        ...
        <div className="relative px-3 py-2">
          ...
          {/* XP Progress Section - Aligned with badges above */}
          <div className="space-y-1">
            ...
            {/* Enhanced Progress Bar - Width matches the badges container above */}
            <div className="relative h-1.5 bg-slate-700/40 rounded-full overflow-hidden border border-slate-600/20">
              <div
                className="absolute top-0 left-0 h-full bg-gradient-to-r from-green-400 via-emerald-500 to-green-600 transition-all duration-700 ease-out rounded-full shadow-sm"
                style={{ width: `${progressPercentage}%` }}
              />
              {/* Shine effect */}
              <div 
                className={`absolute top-0 left-0 h-full bg-gradient-to-r from-transparent via-white/15 to-transparent rounded-full ${isHovered ? 'shimmer-effect' : ''}`}
                style={{ 
                  width: '30%',
