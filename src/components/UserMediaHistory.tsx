...
const UserMediaHistory: React.FC<UserMediaHistoryProps> = ({ className = '' }) => {
  ...
  const userMedia = getUserMediaHistory(user.id)
  ...
  const getTypeIcon = (type: 'speedrun' | 'screenshot' | 'achievement') => {
    switch (type) {
      case 'speedrun':
  ...
  const getTypeColor = (type: 'speedrun' | 'screenshot' | 'achievement') => {
    switch (type) {
      case 'speedrun':
    ...
    <div className={`${className} bg-slate-800 rounded-lg`}>
      ...
      <div className="max-h-96 overflow-y-auto">
        {filteredAndSortedMedia.map((media) => (
          <div key={media.id} className="p-4 border-b border-slate-700/50 hover:bg-slate-750 transition-colors">
