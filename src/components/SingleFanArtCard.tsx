...
const SingleFanArtCard: React.FC<SingleFanArtCardProps> = ({ fanArtItems, className = '' }) => {
  ...
  const handleDismiss = () => {
    if (isAnimating || fanArtItems.length === 0) return
    ...
    setTimeout(() => {
      setCurrentIndex((prev) => {
        ...
        return nextIndex >= fanArtItems.length ? 0 : nextIndex
      })
  ...
  const currentItem = fanArtItems[currentIndex]
        ...
        <div className={`swipeable-card bg-gradient-to-br from-rose-600/10 to-pink-600/10 border-l-4 border-rose-400 relative transition-all duration-300 ${isAnimating ? 'scale-95 opacity-80' : 'scale-100 opacity-100'}`}>
          ...
          <div className="swipeable-card-header">
            ...
            <div className="text-xs text-slate-400">
              <span className="text-rose-400">{currentItem.game}</span>
            </div>
          </div>
          ...
          <div className="swipeable-card-content">
            <div className="p-3 h-full flex flex-col">
              <div className="flex-1 mb-2">
                ...
                <p className="text-xs text-blue-400 mb-1">{currentItem.artist}</p>
                <p className="text-xs text-slate-400">{currentItem.game}</p>
              </div>
              <div className="flex items-center justify-between text-xs text-slate-400 mt-auto">
                <div className="flex items-center gap-2">
                  <span className="flex items-center gap-1">
                    <Heart className="w-3 h-3" /> {currentItem.likes}
                  </span>
                  <span className="flex items-center gap-1">
